...
def get_prediction(df):
    ...
    try:
        ...
        x = temp_df['日期_ts'].values
        y = temp_df['体重'].values.astype(float)
        slope, intercept = np.polyfit(x, y, 1)
        
        if slope < -0.001:  # 体重下降趋势
...
def analyze_food_with_gemini(uploaded_file, g_key):
    ...
    payload = {"contents": [{"parts": [{"text": "识别图片食物热量和纤维素，给小夏温柔建议。"}, {"inline_data": {"mime_type": "image/jpeg", "data": img_b64}}]}]}
    try:
        res = requests.post(url, json=payload, timeout=15).json()
        return res["candidates"][0]["content"]["parts"][0]["text"]
    except: return "识别暂时不可用。"
...
with st.sidebar:
    ...
    st.divider()
    if st.button("📥 导出全量原始数据"):
        if st.session_state.daily_logs:
            df_export = pd.DataFrame(st.session_state.daily_logs)
            st.download_button("点击下载 CSV", df_export.to_csv(index=False).encode('utf-8'), "love_data.csv", "text/csv")
...
with tab1:
    ...
    with col_l:
        if current_user == "小夏":
            st.markdown("#### 🥦 小夏识食 (不留痕)")
            food_img = st.file_uploader("上传饮食照片", type=["jpg", "jpeg", "png"])
            if food_img:
                st.image(food_img, width=250)
                if st.button("开始 AI 识别"):
                    with st.spinner("识别中..."):
                        st.session_state.temp_food = analyze_food_with_gemini(food_img, gemini_key)
            if "temp_food" in st.session_state: st.info(st.session_state.temp_food)
        ...
        with st.form("daily_form_master", clear_on_submit=True):
            ...
            if st.form_submit_button("同步数据"):
                final_detail = f"【AI分析】:{st.session_state.temp_food}\n{detail}" if current_user=="小夏" and "temp_food" in st.session_state else detail
        ...
        if st.button("生成综合审计报告"):
            if deepseek_key and st.session_state.daily_logs:
                with st.spinner("复盘中..."):
                    ...
                    prompt = f"你是小耗子。分析小夏近10天数据：{history_str}。斜率{slope:.3f}。请结合纤维摄入与排便给出温暖分析。" if current_user == "小夏" else f"你是小夏。分析小耗子近10天勤奋度。"
                    res = client.chat.completions.create(model="deepseek-chat", messages=[{"role": "user", "content": prompt}])
                    st.markdown(f'<div class="report-box">{res.choices[0].message.content}</div>', unsafe_allow_html=True)

with tab2:
    if current_user == "小夏":
        st.markdown("### 📉 目标：55.0 kg")
        ...
        if not df_w.empty:
            ...
            c2.metric("距离 55kg", f"{round(current_w-55.0, 1)} kg", delta=f"{slope:.3f} kg/d", delta_color="inverse")
            c3.metric("预测达标日", str(pred_date) if isinstance(pred_res, datetime.date) else "计算中")

            fig = px.line(calc_df, x="日期", y="体重", markers=True, color_discrete_sequence=['#ff6b81'])
            fig.add_hline(y=55.0, line_dash="dot", line_color="green")
            st.plotly_chart(fig, use_container_width=True)
        
        with st.form("w_form_new"):
            st.markdown("#### ⚖️ 更新数据")
            col_a, col_b = st.columns(2)
            val = col_a.number_input("体重 (kg)", 60.0, step=0.1)
...
with tab3:
    st.image("https://images.unsplash.com/photo-1540959733332-eab4deabeeaf?auto=format&fit=crop&w=1200", caption="2026, 东京之约", use_container_width=True)
    st.markdown("### 🎒 冒险清单\n- [ ] 参加花火大会\n- [ ] 穿着浴衣散步")

with tab4:
    if st.text_input("Access Code", type="password") == "wwhaxxy1314":
