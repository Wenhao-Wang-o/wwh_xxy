...
st.markdown("""
    ...
    @media (prefers-color-scheme: dark) {
        ...
        .diary-card { background-color: #2d2d2d !important; color: #efefef !important; border-left: 5px solid #ff8fa3 !important; }
        .reply-card { background-color: #1e2530 !important; color: #d1d1d1 !important; border-left: 5px solid #6a89cc !important; }
        .kcal-box { background: #332b00 !important; border: 1px solid #fcc419 !important; color: #ffd43b !important; }
        p, span, label, .stMarkdown { color: #dddddd !important; }
        .stButton>button { background-color: #ff8fa3 !important; }
        /* 针对输入框在夜间模式的底色优化 */
        .stTextArea textarea, .stNumberInput input { background-color: #262626 !important; color: white !important; border: 1px solid #444 !important; }
    }
    </style>
            ...
            for log in st.session_state.daily_logs[:15]:
                ...
                with st.expander(f"{owner} {log['log_date']} - 心情: {log['mood']}"):
                    ...
                    with c1:
                        ...
                        # 显示评论 (共用 comment_from_haozhi 字段，但文案自适应)
                    ...
                    with c2:
                        # 互动按钮逻辑：如果当前登录用户不是记录本人，则显示评论框
                        if current_user != log['user_name']:
                            ans = st.text_area("回复TA", key=f"ans_{log['id']}", placeholder="写下你的鼓励...")
                            if st.button("发送回复", key=f"b_{log['id']}"):
    ...
    with col_r:
        ...
        if st.button("🚀 生成小夏专项分析", use_container_width=True):
            if api_key_input and st.session_state.daily_logs:
                ...
                history_str = "\n".join([f"- {l['log_date']}: 备注[{l['detail']}]" for l in xia_logs])
                client = OpenAI(api_key=api_key_input, base_url="https://api.deepseek.com")
                res = client.chat.completions.create(
                    model="deepseek-chat", 
                    messages=[{"role": "system", "content": "你是个温柔理性的理科男伴侣。"},{"role": "user", "content": f"分析数据并给小夏贴心建议：\n{history_str}"}]
                )
...
with tab2:
    if current_user == "小夏":
        ...
        if 'weight_data_list' in st.session_state and st.session_state.weight_data_list:
            ...
            current_w_val = calc_df['体重'].iloc[-1]
            c1.metric("当前斜率", f"{slope:.3f} kg/d")
            c2.metric("距离目标", f"{round(current_w_val - 55.0, 1)} kg", delta=f"{slope:.3f}", delta_color="inverse")
...
with tab4:
    ...
    if auth == "wwhaxxy1314":
        st.balloons()
        st.markdown('<div class="diary-card">🌸 宝儿：跨年快乐！...（此处省略正文内容，已在代码中完整保留）</div>', unsafe_allow_html=True)
